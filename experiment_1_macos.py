...
def main():
    ...
    for i in range(num_of_trials):
        ...
        if current_decrypted_message == message:
            ...
            no_crt_decrypt_timings.append(current_time)
    ...
    for i in range(num_of_trials):
        ...
        if current_decrypted_message == message:
            current_end_time = time.time()
            current_time = current_end_time - current_start_time
            crt_decrypt_timings.append(current_time)
            print("Trial ", i + 1, " = ", current_time, " seconds", sep="")
        else:
            print("ERROR: decrypted ciphertext does not match original message", sep="")
            sys.exit(0)
    ...
    print("RSA decryption times without CRT acceleration (in seconds): ", no_crt_decrypt_timings, sep="")
    print("RSA decryption times with CRT acceleration (in seconds): ", crt_decrypt_timings, "\n", sep="")
    no_crt_decrypt_avg = sum(no_crt_decrypt_timings) / len(no_crt_decrypt_timings)
    crt_decrypt_avg = sum(crt_decrypt_timings) / len(crt_decrypt_timings)
    ...
    t, p = stats.ttest_ind(no_crt_decrypt_timings, crt_decrypt_timings)
    print("p-value = ", p, sep="")
    if p < 0.01:
